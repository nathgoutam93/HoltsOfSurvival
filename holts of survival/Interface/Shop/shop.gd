...
func _ready():
	...
	$ColorRect/VBoxContainer/HBoxContainer/close_shop.connect("pressed", self, "_hide")
...
func _set_shop():
	...
	for building in Constants.Buildings.keys():
		if building != Constants.Building.TOWN_HALL:
			...
			$ColorRect/VBoxContainer/HScrollBar/HBoxContainer.add_child(button)

func _reset_shop():
	for children in $ColorRect/VBoxContainer/HScrollBar/HBoxContainer.get_children():
		children.queue_free()
